...
public class ConsultaRellevanciaTest {
    ...
    public void ConsultaRellevanciaProvaModePrimer() {
        ...
        /** Cal comentar primer que el mode 2 és, en essència, una cerca per semblança la qual agafa K=k i el document referència s'aconsegueix
        * amb fent el títol, l'autor i el contingut la nostra query. Per construcció el mode 2 funcionarà, i quedarà comprovat, al comprovar-se la cerca per Semblança (ja tested). */

        //Init

        /** Com que la Classe ConsultaRellevancia només té un metode trivial (que retorna la llibreria que s'ha construit a la constructora)
        ...
        /** El que ara anem a fer és construir una serie de 6 documents (cadascun de dues frases per fer el test viable i no excesivament enorme).
         * Per fer-ho n'hi han parametres trivials (títol, autor, preferit [no intervé en la cerca], path [no intervé en la cerca] i Data [no intervé en la cerca])
         * i per altra banda el contingut. Per crear aquest necessitem dues frases. Per crear-les, però, no podem fer-ho directament mitjançant la constructora que utilitza un 
         * string ja totes les paraules tindran IDs diferents (fins i tot si són la mateixa) cosa que dificultarà molt trobar el nombre d'aparicions d'una paraula
         * determinada en una frase; per això s'insereixen totes en un Vocabulari (on paraules iguals tenen IDs iguals) així com les de la query.
         */
        ...
        query[2] = paraules.inserirObtenirParaula("tal");  //Ningú
        /** Ha de sortir el 1 (ja que k_docs primer el posarem a 1).
        * L'atribut frase només ens importa pel mode 2. */
        ConsultaRellevancia consulta = new ConsultaRellevancia(1,query, "", 1, documents);

        /** Amb la k = 1 només sortirà Doc1 (l'unic amb valor 2). */
        ...
        /** Amb la k = 3 sortiran l'1 i després tenim un triple empat (resolt alfabèticament) -> Sortiran 2 i 5 */
        ...
        /** Amb la k = 5 sortiran l'1 i després tenim el triple empat i per últim (tenim Doc 3 i Doc 4 amb count = 0) el 3 (per alfabètic). */
        ...
        * Agafem 3 per deixar una bona proporció de documents fora. */
        ...
        /** Docs: d5-7, d1-3, d2-3, d3-1, d4-1, d6-1       (suma d'ocurrències) */
        query[2] = paraules.inserirObtenirParaula("que");
        /** Docs: d5-8, d1-3, d2-3, d3-1, d4-1, d6-1       (suma d'ocurrències) */
        query[3] = paraules.inserirObtenirParaula("frase");
        /** Docs: d5-7, d1-4, d2-3, d4-2, d3-1, d6-1       (suma d'ocurrències) */
        query[4] = paraules.inserirObtenirParaula("no");
        /** Docs: d5-7, d1-3, d2-3, d3-3, d4-1, d6-1       (suma d'ocurrències) */
        query[5] = paraules.inserirObtenirParaula("arbre");
        /** Docs: d5-7, d1-3, d2-3, d3-1, d4-1, d6-1       (suma d'ocurrències) */
        query[6] = paraules.inserirObtenirParaula("primer");
        /** Docs: d5-7, d1-4, d2-3, d3-1, d4-2, d6-1       (suma d'ocurrències) */
        query[7] = paraules.inserirObtenirParaula("hola");
        /** Docs: d5-7, d1-5, d2-4, d3-1, d4-1, d6-2       (suma d'ocurrències) */
        consulta = new ConsultaRellevancia(3,query, "", 1, documents);
        resultat = new Llibreria();
